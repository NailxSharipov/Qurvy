...
impl EditorApp {
    pub(crate) fn intersect_control(&self) -> Column<AppMessage> {
            ...
            Row::new()
                ...
                .push(
                    Container::new(
                        pick_list(
                            &SolverOption::ALL[..],
                            Some(self.state.intersect.solver),
                            on_select_solver,
            ...
            Row::new()
                ...
                .push(
                    Container::new(
                        pick_list(
                            &FillOption::ALL[..],
                            Some(self.state.intersect.fill),
                            on_select_fill,
                        ).width(Length::Fixed(160.0))
                    )
                        .height(Length::Fill)
            ...
            Row::new()
                ...
                .push(
                    Container::new(
                        pick_list(
                            &ModeOption::ALL[..],
                            Some(self.state.intersect.mode),
                            on_select_mode,
                        ).width(Length::Fixed(160.0))
                    )
                        .height(Length::Fill)
...
fn on_select_fill(option: FillOption) -> AppMessage {
    AppMessage::Intersect(IntersectMessage::FillSelected(option))
}

fn on_select_mode(option: ModeOption) -> AppMessage {
    AppMessage::Intersect(IntersectMessage::ModeSelected(option))
}

fn on_select_solver(option: SolverOption) -> AppMessage {
    AppMessage::Intersect(IntersectMessage::SolverSelected(option))
}
